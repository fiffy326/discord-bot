...
const command: Command = {
  data: new SlashCommandBuilder()
    ...
    .addNumberOption(option =>
      option.setName("min").setDescription("Minimum"))
    .addNumberOption(option =>
      option.setName("max").setDescription("Maximum"))
  ...
  async execute(interaction: any) {
    ...
    if (minimum < 0) {
      interaction.reply({ content: "The minimum must be >= 0.", ephemeral: ephemeral });
      return;
    }

    if (maximum > Number.MAX_SAFE_INTEGER) {
      interaction.reply({ content: `The maximum must be <= ${Number.MAX_SAFE_INTEGER}.`, ephemeral: ephemeral });
      return;
    }
