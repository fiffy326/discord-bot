...
async function deployGlobalCommands(): Promise<void> {
  const payload = (await loadCommandFiles()).map((command) => command.data.toJSON());
  await rest
...
async function deleteGuildCommands(guildIds?: string[]): Promise<void> {
  guildIds = guildIds ?? config.guilds.filter((guild) => guild.deploy_commands).map((guild) => guild.id) ?? [];
